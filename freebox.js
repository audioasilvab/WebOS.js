...
(function() {
 ...
 self.max &&*/
    ...
    function U(a, b) {
        ...
        T && (b = this.g.getElementsByClassName("wb-header")[0],
        u(b, "height", T + "px"),
    ...
    ;
    function V(a, b, c) {
        "string" === typeof a && ("center" === a ? a = (b - c) / 2 + .5 | 0 : "right" === a || "bottom" === a ? a = b - c : (c = parseFloat(a),
        a = "%" === ("" + c !== a && a.substring(("" + c).length)) ? b / 100 * c + .5 | 0 : c));
        return a
    }
    function ja() {
        B = document.body;
    ...
    function W(a, b) {
        function c(g) {
            ...
            a.min || (B.classList.add("wb-lock"),
            (p = g.touches) && (p = p[0]) ? (g = p,
            k(window, "touchmove", f, m),
            k(window, "touchend", d, m)) : (k(window, "mousemove", f, m),
            k(window, "mouseup", d, m)),
            v = g.pageX,
            L = g.pageY)
        }
        function f(g) {
            ...
            H && (a.width = Math.max(Math.min(a.width, a.m, K - a.x - a.right), a.s),
            H = a.width !== C);
            I && (a.height = Math.max(Math.min(a.height, a.l, P - a.y - a.bottom), a.o),
            I = a.height !== y);
            (H || I) && a.resize();
            G && (a.max && (a.x = (q < K / 3 ? a.left : q > K / 3 * 2 ? K - a.width - a.right : K / 2 - a.width / 2) + r),
            a.x = a.x,
            G = a.x !== z);
            D && (a.max && (a.y = a.top + w),
            a.y = a.y,
    ...
    e.setUrl = function(a, b) {
        ...
        c && "iframe" === c.tagName.toLowerCase() ? c.src = a : (this.body.innerHTML = '<iframe src="' + a + '"></iframe>',
    ...
    e.blur = function(a) {
        if (!1 === a)
            return this.focus();
        this.focused && (this.removeClass("focus"),
        this.focused = !1,
    ...
    // Si está oculta, restaurar
    if (this.hidden) {
        // Restaurar posición guardada
        if (this._minimizedState) {
            this.move(this._minimizedState.x, this._minimizedState.y);
            this.resize(this._minimizedState.width, this._minimizedState.height);
        }
        ...
        // Remover de la lista de minimizadas
        var idx = x.indexOf(this);
        if (idx !== -1) x.splice(idx, 1);
    }
    
    // Siempre traer al frente y enfocar (incluso si ya estaba visible)
    this.focus();
    ...
    // Llamar al minimize original para mantener eventos
    // Pero como estamos sobrescribiendo, implementamos manualmente
    F && qa();
    ...
    e.maximize = function(a) {
        ...
        F && qa();
        this.min && pa(this);
        this.max || (this.addClass("max").resize(K - this.left - this.right, P - this.top - this.bottom, !0).move(this.left, this.top, !0),
        this.max = !0,
    ...
    e.move = function(a, b, c) {
        a || 0 === a ? c || (this.x = a ? a = V(a, K - this.left - this.right, this.width) : 0,
        this.y = b ? b = V(b, P - this.top - this.bottom, this.height) : 0) : (a = this.x,
        b = this.y);
        u(this.g, "left", a + "px");
        u(this.g, "top", b + "px");
    ...
    e.resize = function(a, b, c) {
        a || 0 === a ? c || (this.width = a ? a = V(a, this.m) : 0,
        this.height = b ? b = V(b, this.l) : 0,
        a = Math.max(a, this.s),
        b = Math.max(b, this.o)) : (a = this.width,
        b = this.height);
        u(this.g, "width", a + "px");
